-- Migration: create_kuwait_exam_tables
-- Created at: 1762622600

-- Kuwait English Learning Platform - Exam System Tables
-- Enable UUID extension
CREATE EXTENSION IF NOT EXISTS "uuid-ossp";

-- 1. KUWAIT EXAM COMPONENTS
...
CREATE TABLE kuwait_user_progress (
  id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  user_id UUID NOT NULL,
  component_id UUID NOT NULL,
  grade_level TEXT NOT NULL CHECK (grade_level IN ('10', '11', '12')),
...
ALTER TABLE kuwait_user_progress ENABLE ROW LEVEL SECURITY;

-- RLS Policies - Public Read Access (No Authentication Required)
CREATE POLICY "Public read exam_components" ON exam_components FOR SELECT USING (true);
...
CREATE POLICY "Public read assessments" ON assessments FOR SELECT USING (is_published = TRUE);
CREATE POLICY "Public read submissions" ON submissions FOR SELECT USING (true);
CREATE POLICY "Public read user_progress" ON kuwait_user_progress FOR SELECT USING (true);
...
CREATE INDEX idx_kuwait_progress_user ON kuwait_user_progress(user_id);;
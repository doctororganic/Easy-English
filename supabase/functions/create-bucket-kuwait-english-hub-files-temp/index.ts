...
Deno.serve(async (req) => {
    ...
    const bucketConfig = {
        id: 'kuwait-english-hub-files',
        name: 'kuwait-english-hub-files',
        public: true
    };

    // Add optional configurations
    bucketConfig.allowed_mime_types = ["application/pdf", "application/msword", "application/vnd.openxmlformats-officedocument.wordprocessingml.document", "text/plain", "image/*"];
    ...
    const policyQueries = [
        ...
        `CREATE POLICY "Public Access for kuwait-english-hub-files" ON storage.objects FOR SELECT USING (bucket_id = 'kuwait-english-hub-files');`,
        // Allow public to insert objects in this bucket
        `CREATE POLICY "Public Upload for kuwait-english-hub-files" ON storage.objects FOR INSERT WITH CHECK (bucket_id = 'kuwait-english-hub-files');`,
        // Allow public to update objects in this bucket
        `CREATE POLICY "Public Update for kuwait-english-hub-files" ON storage.objects FOR UPDATE USING (bucket_id = 'kuwait-english-hub-files');`,
        // Allow public to delete objects in this bucket
        `CREATE POLICY "Public Delete for kuwait-english-hub-files" ON storage.objects FOR DELETE USING (bucket_id = 'kuwait-english-hub-files');`
    ];
    ...
    return new Response(JSON.stringify({
        ...
        name: 'kuwait-english-hub-files',
        public: true,
        allowed_mime_types: ["application/pdf", "application/msword", "application/vnd.openxmlformats-officedocument.wordprocessingml.document", "text/plain", "image/*"],
